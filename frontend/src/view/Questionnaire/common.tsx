...
import { indexToBullet } from 'src/modules/utils';
...
const getScoreFromAnswers = (section) => {
  if (
    !section.type ||
  ...
  const rightAnswers = (section.rightAnswer ?? []).sort();
  const answers = (toArray(section.answer) ?? []).sort();
